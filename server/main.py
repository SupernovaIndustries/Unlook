...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('/var/log/unlook/server.log', mode='a')
    ]
)
logger = logging.getLogger("UnLookServer")

# Assicura che la directory per i log esista
os.makedirs('/var/log/unlook', exist_ok=True)
...
try:
    ...
    logger.error("Installa le dipendenze necessarie con: pip install pyzmq numpy picamera2")
    sys.exit(1)
...
class UnLookServer:
    ...
    def _generate_device_id(self) -> str:
        ...
        id_file = Path('/etc/unlook/device_id')
        ...
        # Salva l'ID nel file
        os.makedirs('/etc/unlook', exist_ok=True)
    ...
    def _capture_frames(self) -> bool:
        ...
        try:
            ...
            capture_dir = Path('/var/lib/unlook/captures')
